...
def safe_rerun():
    try:
        ...
        # For Streamlit versions where experimental_rerun is removed
...
# --- Extract JSON from GenAI response text robustly ---
...
if "page" not in st.session_state:
    st.session_state.page = "input"  # 'input' or 'recommendation'
...
def show_input_form():
    ...
    if mode == "Web Form (Structured)":
        with st.form("input_form"):
            ...
            workload_type = st.selectbox("Workload Type", ["Batch", "Service", "AI/ML", "Storage", "Other"], help="Type of workload")
        ...
        st.write("Example: 'Need 8 vCPUs, 100GB storage, NVIDIA T4 GPU for AI hackathon in us-central1 from Aug 20-Sept 5, budget $1500.'")
...
def show_recommendation_page():
    ...
    if not st.session_state.genai_result:
        with st.spinner("Generating recommendation..."):
            ...
            st.session_state.genai_result = result
            config = result.get("config", {})
            st.session_state.editable_rec = dict(config)

    result = st.session_state.genai_result
    ...
    for field, opts in editable_fields.items():
        if field in editable_rec:
            val = st.slider(
                ...
                min_value=opts["min"],
                max_value=opts["max"],
                value=int(editable_rec[field]),
                step=opts["step"],
                key=f"slider_{field}",
            )
            editable_rec[field] = val
    ...
    with cols[0]:
        if st.button("Back to Input"):
            # Clear recommendation and editable config so input page resets
...
if st.session_state.page == "input":
    ...
    st.error(f"Invalid page state: {st.session_state.page}")

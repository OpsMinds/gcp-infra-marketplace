...
# Your Service Account key as a Streamlit secret or environment variable
# You must have this file or its content in your environment.
SERVICE_ACCOUNT_FILE = st.secrets.get("service_account_file_path", None)
BILLING_ACCOUNT_ID = st.secrets.get("billing_account_id", None)

# Initialize Google Cloud Billing Pricing API
@st.cache_resource(ttl=3600)
def init_billing_client():
    if not SERVICE_ACCOUNT_FILE:
        st.error("Service account file path not found in secrets. Pricing API unavailable.")
        return None
...
# Fetch SKU prices per category from billing account
    ...
    try:
        while True:
            ...
            for sku in response.get("skus", []):
                ...
                # Only interested in Compute Engine SKUs info related to CPU, Memory, Storage, GPU
                if "Compute Engine" in service_name:
                    # SKU pricing info can have tiered rates, pick the first tier on-demand price
                    for price_info in sku.get("pricingInfo", []):
                        effective_time = price_info.get("effectiveTime")
                        ...
                        if tiered_rates:
                            ...
                            # Key SKU uniquely by description + region + usage_type
...
# Function to map user resources to SKU price keys, then get per unit prices
def get_price_for_resource(resource_desc, region, usage_type, skus_price_map):
    # Typical SKU description matching strings
...
# Prompt to extract fields in Chatbot input
def make_extraction_prompt(free_text, env, purpose):
    return (
        ...
        "If any field is missing, try to infer from context, else use empty string. Dates as 'YYYY-MM-DD', numbers as int.\n"
        f"\nUser text:\n{free_text}\n"
        f"Environment: {env}\nPurpose: {purpose}\n"
...
def build_prompt(fields: dict) -> str:
    prompt = (
        ...
        "- config: a dictionary with final suggested configurations including vCPUs, memory, storage, GPU, budget, and other fields.\n"
...
# Estimate billing cost in real-time using GCP Billing Pricing API
def estimate_billing_cost_from_api(config, region, skus_price_map):
    ...
    # SKU usage types and SKU description snippets for matching
    # This might need adjustment depending on actual SKU descriptions you see
    cpu_desc = "N1 Predefined Instance Core"  # or "Compute Engine Core" etc
    memory_desc = "N1 Predefined Instance Ram"
    storage_desc = "SSD"  # or "Standard Persistent Disk"
    gpu_desc_map = {
        "None": None,
        "NVIDIA T4": "NVIDIA T4",
    ...
    # UsageType 'OnDemand' or 'Commit1Mo' etc can be filtered if desired
    usage_type = "OnDemand"

    # Find prices or fallback to zero
    ...
    # Prices are per hour per unit, multiply by usage and hours in period
    hours = days * 24

    total_cost = (
        (cpu_price * vcpu_count) +
        (mem_price * memory_gb) +
...
# Initialize billing client and pricing SKU map once
if not st.session_state.billing_client:
    st.session_state.billing_client = init_billing_client()
if st.session_state.billing_client and not st.session_state.skus_price_map:
        ...
        if st.button("Submit Chatbot"):
            if raw_text.strip():
                ...
                if "error" in parsed_fields:
                    ...
                    # Merge environment and purpose for safety
...
def show_recommendation_page():
    ...
    # Estimate billing with real GCP pricing API
    region = editable_rec.get("region", "us-central1")  # default region if unset
